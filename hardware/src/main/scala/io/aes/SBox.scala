...
class SBox extends Module with SBoxValues {
  ...
  def sbox(byte: UInt): UInt = {
    return byte ^ rotl(byte, 1) ^ rotl(byte, 2) ^ rotl(byte, 3) ^ rotl(byte, 4) ^ 0x63.U
  }
...
object SBoxMain extends App {
  println("Generating the AES hardware")
  chiselMain(Array("--targetDir", "generated"), () => Module(new SBox()))
}