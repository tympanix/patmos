...
class AesTester(dut: Aes) extends Tester(dut) {
  ...
  val KEY_VALUES = List(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
  val RESULT_VALUES = List(0x03, 0x36, 0x76, 0x3e, 0x96, 0x6d, 0x92, 0x59, 0x5a, 0x56, 0x7c, 0xc9, 0xce, 0x53, 0x7f, 0x5e)

  val testBlock = BLOCK_VALUES.map(v => UInt(v, width = 8))
  val testKey = KEY_VALUES.map(v => UInt(v, width = 8))
  val testOut = RESULT_VALUES.map(v => UInt(v, width = 8))
